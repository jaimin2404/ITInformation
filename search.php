    ...
    <div class="main index">
        <div class="main-content">
            ...
            <?php
                ...
                $offset = ($page - 1) * $limit;
                $sql = "SELECT post.postId,post.postTitle,post.postDescription,post.postDate,post.postImage,post.postAuthor,category.categoryId,category.categoryName,user.userName,user.name FROM post LEFT JOIN category ON post.postCategory = category.categoryId LEFT JOIN user ON post.postAuthor = user.userId WHERE post.postTitle LIKE '%{$searchTerm}%' OR post.postDescription LIKE '%{$searchTerm}%' ORDER BY post.postId DESC LIMIT {$offset},{$limit}";
                ...
                $qryPagination = "SELECT * FROM post  WHERE post.postTitle LIKE '%{$searchTerm}%' OR post.postDescription LIKE '%{$searchTerm}%'";
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    echo "<ul>";
                    if ($page > 1) {
                        echo '<li><a href="search.php?search='.$searchTerm.'&page=' . ($page - 1) . '">Prev</a></li>';
                    }
                    ...
                    if ($totalPage > $page) {
                        echo '<li><a href="search.php?search='.$searchTerm.'&page=' . ($page + 1) . '">Next</a></li>';
                    }
                    echo "</ul>";
                    echo "</div>";
                    echo "</div>";
